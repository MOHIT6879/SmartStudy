    ...
    try {
        ...
        if (isSupabaseConfigured()) {
            ...
            if (dbErr) {
                ...
                delete legacyAssignment.subject;
                delete legacyAssignment.language;
                delete legacyAssignment.difficulty;
                delete legacyAssignment.duration_minutes;
...
app.post('/api/submissions', uploadDisk.any(), async (req, res) => {
    ...
    try {
        ...
        const pdfEval = await evaluateStudentAnswerAgainstPdf(ocrText, targetClassName, imageBuffers, primaryFile?.mimetype || 'image/jpeg', assignedQuestions, reasoningModel, lang);
        ...
        if (isSupabaseConfigured()) {
            ...
            if (updateError)
                throw new Error(`Could not save submission evaluation: ${updateError.message}`);
        ...
        if (persistedSubmissionId && isSupabaseConfigured()) {
            ...
            if (failureUpdateError)
                console.warn('⚠️ Could not mark submission as failed:', failureUpdateError.message);
        }
        res.status(500).json({ success: false, message: err.message });
    }
});
...
app.post('/api/submissions/bulk', uploadDisk.any(), async (req, res) => {
    try {
        ...
        const batchJob = createBatchJob(batchFiles, assignmentId, selectedLanguage || 'English', className, subject);
        res.json({
            success: true,
            jobId: batchJob.id,
