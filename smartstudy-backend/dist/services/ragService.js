 ...
 */
function createSemanticParagraphChunks(rawText, targetLength = 700, maxChunks = 350) {
    if (!rawText)
        return [];
    console.log(`\n===============================================================`);
    ...
    if (chunks.length < 5) {
        ...
        if (buf.length > 20 && chunks.length < maxChunks)
            chunks.push(buf);
    }
 ...
 */
export async function ingestPdfDocument(filesInput, className, chapterTitle, fileName) {
...
Key Learning Objectives: Master core terminology, understand foundational theories, analyze key principles, and synthesize subject knowledge for ${className}.`;
    ...
    try {
        ...
        for (let i = 0; i < chunks.length; i += batchSize) {
            ...
            const batchRecords = await Promise.all(chunkBatch.map(async (chunk) => {
