 ...
 */
async function processBatchQueue(jobId, files, assignmentId, selectedLanguage = 'English', targetClassName, targetSubject) {
    ...
    const worker = async () => {
        while (queue.length > 0) {
            ...
            try {
                ...
                const pdfEval = await evaluateStudentAnswerAgainstPdf(ocrText, className, fileItem.buffer, fileItem.mimetype);
